...
| Account Type | Selector Card | Username ID | Password | Access Level |
| :--- | :--- | :--- | :--- | :--- |
| **👑 Administrator** | **Left Card** | `Admin` | **`Shravan`** | Master Administrator (Full Access) |
| **🛡️ Employee** | **Right Card** | `Rudra` | **`RudraSarika@2505`** | Operations & Data Entry |
...
Both **Admin** and **Rudra** have full operational access to manage day-to-day transport operations, advance disbursements, custom sections, live Excel sheets, and settings. 

The **only distinction** is on account password administration:
* **👑 Admin** has master control and can change passwords for both `Admin` and `Rudra`.
* **👤 Rudra** has full operational permissions and can change Rudra's own password.
...
| **Delete Transport** | ✅ | ✅ | Remove erroneous records with confirmation dialog |
...
| **Delete Advance** | ✅ | ✅ | Remove advance entries with confirmation dialog |
...
| **Delete Custom Section** | ✅ | ✅ | Remove custom sections (Sections 1 & 2 remain base protected) |
| **Live Excel Sheet View** | ✅ | ✅ | 1:1 Shinex Excel replica with click-to-edit row and zoom scaling |
| **Excel Export (.xlsx)** | ✅ | ✅ | Cell-by-cell AutoFit width download with offline engine |
...
```text
             ...
             ▼                                                           ▼
┌───────────────────────────────┐               ┌───────────────────────────────┐
│     👑 Admin Workstation       │               │      🛡️ Rudra Workstation     │
...
1. **Rudra enters data:** Rudra creates or edits a trip or advance on her workstation.
...
5. **Admin Workstation Polling:** Admin's browser automatically queries the cloud backend (3-second background polling cycle + window focus trigger), retrieves the new records, replaces the in-memory dataset, and recalculates all ledger balances in real time.
6. *(Optional same-browser optimization)*: `BroadcastChannel('shinex_sync_channel')` notifies any other tabs open on the exact same computer instantly.
...
* **Root Cause:**
  The project originally relied on browser `localStorage`. Because `localStorage` is isolated to a single browser profile, changes on Rudra's computer were completely invisible to Admin's computer, causing stale numbers and missing live calculations.
* **Fix Applied:**
  - Standardized Google Sheets as the single source of truth via `backend/Code.gs`.
  - Added automatic 3-second background polling and window-focus synchronization in `js/app.js`.
  - Added `BroadcastChannel` for same-machine multi-tab notification.
...
* **Root Cause:**
  ...
  - Cloud latency race condition: fetching before Google Apps Script finished writing caused older data to overwrite newly entered rows.
* **Fix Applied:**
...
### 3. Exact 1:1 Live Excel Spreadsheet Replica
...
### 4. Section-Divided Advances Ledger
...
### 5. Bulletproof Excel (.xlsx) Downloads with AutoFit
...
├── css\
│   └── styles.css        # Responsive layout, left sidebar, user switch cards & employee isolation
├── js\
...
5. Click **Deploy** ➔ **New deployment**:
   - **Type:** Web app
   - **Description:** Shinex Transport API v4.7
   - **Execute as:** `Me`
